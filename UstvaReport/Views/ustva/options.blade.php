    ...
    <script type="text/javascript">
      $(function () {
        $('#btn-run-report').click(function () {
          ...
          $.post("{{ route('addons.reports.ustva.validate') }}", {
            from_date: from_date,
            to_date: to_date,
            ...
            if (output_type == 'preview') {
              $('#preview').show();
              $('#preview-results').attr('src', "{{ route('addons.reports.ustva.html') }}" + query_string);
            }
            else if (output_type == 'pdf') {
              window.location.href = "{{ route('addons.reports.ustva.pdf') }}" + query_string;
            }
...
@section('content')
    <section class="content-header">
        <h1 class="pull-left">{{ trans('UstvaReport::common.title') }}</h1>
